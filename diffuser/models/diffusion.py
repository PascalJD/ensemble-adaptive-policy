...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        action_weight=1.0, loss_discount=1.0, loss_weights=None, returns_condition=False,
        condition_guidance_w=0.1, ar_inv=False, train_only_inv=False, action_dropout=False, action_nll=False):
        super().__init__()
        self.horizon = horizon
        self.observation_dim = observation_dim
        self.action_dim = action_dim
    ...
    def loss(self, x, cond, returns=None):
        if self.train_only_inv:
            ...
            if self.ar_inv:
                ...
                pred_mean, pred_variance = self.inv_model(x_comb_t)
                loss = nll_loss(pred_mean, pred_variance, a_t)
                info = {'a0_loss':loss}
            else:
            ...
            if self.ar_inv:
                ...
                pred_mean, pred_variance = self.inv_model(x_comb_t)
                inv_loss = nll_loss(pred_mean, pred_variance, a_t)
            else:
                pred_a_t = self.inv_model(x_comb_t)
...
class PredictiveUncertaintyModel(nn.Module):
    def __init__(self, observation_dim, hidden_dim, action_dim):
        ...
        self.inv_model = nn.Sequential(
            ...
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 2 * self.action_dim),  # Outputs both mean and variance
        )

    def forward(self, x):
        output = self.inv_model(x)
        mean = output[:, :self.action_dim]
        # ensure positivity using the softplus activation
        variance = F.softplus(output[:, self.action_dim:])
        return mean, variance
    

def nll_loss(mean, variance, y):
    epsilon = 1e-6
    adjusted_variance = variance + epsilon
    precision = 1 / adjusted_variance
    log_variance = torch.log(adjusted_variance)
    squared_error = (y - mean) ** 2
    loss = 0.5 * log_variance + 0.5 * precision * squared_error
